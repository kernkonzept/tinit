...
class Ram_tracker
{
  ...
  {
    Pool() = default;
    constexpr Pool(unsigned long start, unsigned long end, unsigned char nodes,
         Pool *next = 0)
  ...
  struct Range
  {
    ...
    Range(unsigned long start, unsigned long end, unsigned nodes)
...
public:
  void add(unsigned long start, unsigned long end, unsigned nodes)
  {
    Range *spot = &list;
    ...
    Range *n = alloc_range(start, end, nodes);
  ...
  void sub(unsigned long start, unsigned long end)
  {
	...
	if (i->end < start)
	...
	else if (i->start < start && end < i->end)
	  {
	    ...
	    Range::insert_after(i, alloc_range(end + 1U, i->end, i->nodes));
	    i->end = start - 1U;
	    break;
	  }
  ...
  void add_pool(unsigned long address, unsigned long size, unsigned nodes)
  {
    pool = new Pool(address, address + size - 1U, nodes, pool);
  }

  unsigned long alloc(unsigned long size, unsigned long align, unsigned nodes)
  {
    static constexpr Pool whole_ram{0, ~0UL, 0xffU, 0};
        ...
        for (Pool const *p = pool ? pool : &whole_ram; p; p = p->next)
          {
            ...
            if (reserve(start, start + size - 1U, nodes, true))
              return start;
          }
      }
   ...
   */
  bool reserve(unsigned long start, unsigned long end, unsigned nodes,
               bool exclusive)
  {
    Range *i = list.next;
    while (i != &list)
    ...
    if ((i->nodes & nodes) == 0)
      {
	Info().printf("Cannot reserve [%lx-%lx]/%x in foreign region [%lx-%lx]/%x\n",
		      start, end, nodes, i->start, i->end, i->nodes);
	return false;
      }
	...
	if (i->start != start && i->end != end)
	  {
	    ...
	    Range::insert_before(i, alloc_range(i->start, start - 1U, i->nodes));
	    Range::insert_after(i, alloc_range(end + 1U, i->end, i->nodes));
	  }
  ...
  {
    for (Range *i = list.next; i != &list; i = i->next)
      dbg.printf("  [%8lx - %8lx], nodes 0x%x, %s\n", i->start, i->end,
                  i->nodes, i->shared ? " (shared)" : "");
  }

private:
  Range *alloc_range(unsigned long start, unsigned long end, unsigned nodes)
  {
    if (Range::empty(&slab))
      return new Range(start, end, nodes);
...
void *Page_alloc::alloc(unsigned long size, unsigned long align,
                        unsigned nodes)
{
  return (void *)ram.alloc(size, align, nodes);
}

bool Page_alloc::reserve(unsigned long address, unsigned long size,
                         unsigned nodes)
{
  return ram.reserve(address, address + size - 1U, nodes, true);
}

bool Page_alloc::share(unsigned long address, unsigned long size)
{
  return ram.reserve(address, address + size - 1U, ~0U, false);
}
