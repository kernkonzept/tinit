                             ...
                             unsigned node)
{
  ...
  elf.iterate_phdr([&task_min, &task_max, &task_align](Loader::Elf_phdr ph, void const *) {
    if (ph.type() != PT_LOAD && ph.type() != PT_L4_STACK)
      return;
    if (!ph.memsz())
      return;
  ...
  l4_addr_t base = (l4_addr_t)Page_alloc::alloc(task_max - task_min,
                                                task_align,
                                                1U << node);
  if (!base)
    {
    ...
    if (ph.type() == PT_LOAD || ph.type() == PT_L4_STACK)
      {
	    ...
	    // XIP
        ...
        else
          {
            ...
            if (!Page_alloc::reserve(dest, size, 1U << l4_kip()->node))
              {
                Fatal fatal;
...
App_task::reserve_ram(cxx::String const &arg0, l4_addr_t reloc, unsigned node)
{
  ...
  bool ret = true;
#ifndef CONFIG_TINIT_DYNAMIC_LOADER
  elf.iterate_phdr([reloc, &ret, node](Loader::Elf_phdr ph, void const *f) {
    if (ph.type() == PT_LOAD || ph.type() == PT_L4_STACK)
      {
        l4_addr_t dest = ph.paddr() + reloc;
        l4_addr_t size = l4_round_page(ph.memsz());
        ...
        if ((ph.flags() & PF_W) || ph.memsz() > ph.filesz()
            || src != (void *)dest)
          {
            if (!Page_alloc::reserve(dest, size, 1U << node))
              ret = false;
          }
      }
  });
